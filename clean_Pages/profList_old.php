...
<head>
    ...
    var map;
      ...
      <?php 
      $content;
  $sql_command = "SELECT `id`, `long`, `lat` FROM  `location`";
  ...
  {
    $sql_command = "SELECT vorname, nachname FROM  location, profile WHERE location.id=profile.id AND location.id =$geo[0]";
    $nameresult = mysqli_query($GLOBALS['link'], $sql_command);
    $names = mysqli_fetch_row($nameresult);

     echo 'var marker'.$geo[0].';';
     echo 'var infoWindowContent'.$geo[0].';';
     echo 'var infowindow'.$geo[0].';';
     $pre;
     $content = $pre;
    $content .= "<b><a href=\"profile.php?id=".$geo[0]."\">View Profile: ".$names[0].' '.$names[1]."</a></b><br>";
    $currentlat=$geo[2];
    $currentlng=$geo[1];
    ...
    while($geo = mysqli_fetch_array($result)){
     ...
     $content .= "<b><a href=\"profile.php?id=".$geo[0]."\">View Profile: ".$names[0].' '.$names[1]."</a></b><br>";
        ...
        $pre = '<b><a href="project.php?id='.$geo[0].'">View Profile: '.$names[0].' '.$names[1].'</a></b><br>';
        break;}
  }


    echo 'infoWindowContent'.$geo[0].' = \''.$content.'\';';

       echo 'infowindow'.$geo[0].' = new google.maps.InfoWindow({
      content: infoWindowContent'.$geo[0].'
        });';
    
  
    echo 'marker'.$geo[0].'=new google.maps.Marker({
          position: new google.maps.LatLng('.$geo[2].', '.$geo[1].'),
          map: map
        });';

    echo '  google.maps.event.addListener(marker'.$geo[0].', \'click\', function() {
            infowindow'.$geo[0].'.open(map,marker'.$geo[0].');
